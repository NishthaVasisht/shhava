...
import { Heart, Sparkles, MapPin, MessageCircle, Shield, Star, Plus, Clock, Award } from 'lucide-react';
...
export default function Dashboard() {
  ...
  const fetchSerendipityMoments = async () => {
    try {
      const response = await fetch('/serendipity-moments');
      const data = await response.json();
  ...
  const fetchFateFlashbacks = async () => {
    try {
      const response = await fetch('/fate-flashbacks');
      const data = await response.json();
      setFateFlashbacks(data.flashbacks?.slice(0, 2) || []); // Show only latest 2
    } catch (error) {
  ...
  const handleViewFlashback = async (id: number) => {
    try {
      ...
      setFateFlashbacks(prev => prev.map(fb => 
  ...
  const createSerendipityMoment = async () => {
    // Placeholder for creating a new serendipity moment
    const newMoment = {
      location_name: "Coffee House, Ludhiana",
      latitude: 30.9000,
      longitude: 75.8500,
      moment_description: "Saw someone reading poetry at the corner table, our eyes met briefly over 'Ghalib' verses",
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-100 via-pink-50 to-blue-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 mt-4 animate-fade-in delay-500" style={{ fontFamily: 'Fredoka, sans-serif' }}>Could this BE loading any slower? 😄</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-100 via-pink-50 to-blue-100">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className={`mb-8 transition-all duration-1000 delay-200 ${isVisible ? 'animate-slide-in-up' : 'opacity-0'}`}>
          ...
          <p className="text-gray-600 text-lg animate-slide-in-left delay-300" style={{ fontFamily: 'Inter, sans-serif' }}>
            Your story's about to get <em>really</em> good... Time for some legendary moments! 🎬
          </p>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            {fateFlashbacks.length > 0 && (
              <div className={`bg-white/80 backdrop-blur-lg rounded-3xl p-8 shadow-xl hover-lift transition-all duration-1000 delay-300 ${isVisible ? 'animate-slide-in-up' : 'opacity-0'}`}>
                ...
                {[
                  ...
                  >
                    <div className="flex items-center space-x-3 mb-3">
                      ...
                      <span className={`text-xs px-2 py-1 rounded-full animate-shimmer ${
